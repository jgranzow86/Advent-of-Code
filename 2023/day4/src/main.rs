...
fn main() {
    ...
    let part1_answer = part1(&cards);
    println!("Part 1: {part1_answer}");

    let part2_answer = part2(&mut cards);
    println!("Part 2: {part2_answer}")
}

fn part1(cards: &[ScratchCard]) -> u64 {
...
fn part2(cards: &mut[ScratchCard]) -> u64 {
    let mut total_card_count = 0;

    for index in 0..cards.len() {
        let mut win_count = 0;
        for drawn_number_index in 0..cards[index].drawn_numbers.len() {
            if cards[index].winning_numbers.contains(&cards[index].drawn_numbers[drawn_number_index]) {
                win_count += 1;
                let next_index = index + win_count;
                cards[next_index].card_count += cards[index].card_count;
            }
        }
    }

    for card in cards {
        total_card_count += card.card_count;
    }
    total_card_count
}
...
impl ScratchCard {
    fn new_from_str(input: &str) -> ScratchCard {
        ...
        let card_count = 1;
        let scratch_card = ScratchCard{id, winning_numbers, drawn_numbers, card_count};
        scratch_card
    }
}
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11";
        ...
        let cards: Vec<ScratchCard> = sample
            .lines()
            .into_iter()
            .map(|line| ScratchCard::new_from_str(line))
            .collect();
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11";
        ...
        let mut cards: Vec<ScratchCard> = sample
        .lines()
        .into_iter()
        .map(|line| ScratchCard::new_from_str(line))
        .collect();

        let sum = part2(&mut cards);
        assert_eq!(sum, 30);
    }
